...
$quad quad$此外，在DNN拟合"摄影师图像"实验中，DNN先拟合轮廓后拟合细节，进一步验证了F-Principle.\
...
PhaseDNN的核心步骤如下：先对数据执行离散时间变换（DTT），再将频域数据划分为若干等频谱长度的子带；对每个子带数据，乘以其中心频率$omega$对应的调谐因子$e^(-i omega x)$，实现高频谱域向低频谱域的迁移；随后通过傅里叶逆变换转换至时域，交由神经网络完成低频特征学习；最后将学习后的特征映射回原始高频域，并对各子带结果进行求和，得到目标函数。

这一方法的缺点是：傅里叶级数的项数会随数据维度的增加呈指数级增长，这导致相位深度神经网络不可避免地面临维数灾难问题。
