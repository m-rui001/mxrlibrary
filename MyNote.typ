...
#align(center)[#set text(size: 36pt,font:"FZShuti") 
前言]
#align[
#set text(size:20pt) 
证明数学命题，既要熟悉对象的性质，也要掌握常见的思想方法。然而写成的证明中，这些思考过程常被隐去。读者逐步阅读时，即使每步都懂，仍可能困惑：“为何他能想到，我却不能？”

纸质教材常因篇幅所限，难以兼顾启发性、系统性与严谨性。其中启发性最易被牺牲，也增加了阅读难度。

本笔记不追求系统全面，而注重知识、方法之间的联系。希望通过一系列例子，提炼贯穿不同数学领域的核心思想与技巧。

让我们一起，踏上这条光辉灿烂的旅途！

本文使用typst编写,感谢简洁的typst语法让数学写作变得如此轻松愉快。
#align(right)[坟头草\
2026年1月21日\
QQ:3900951892
]
...
#image("assets/1890-Almond-Blossoms-scaled.webp")

作者：文森特·梵高 - vincent van gogh\

作品名称：《杏花》\

作品年代：1890年2月 阿尔圣雷米医院


1890年，梵高割耳后被送进圣雷米医院治疗，在那里他跟各种精神病人住在一起，医院的生活条件很差，梵高病情时好时坏。

住院期间，梵高收到弟弟提奥的来信：“妻子乔安娜怀孕了，他要做父亲了”。梵高感动得热泪盈眶，这也许是梵高人生中最幸福的时刻，他最爱的弟弟要做父亲了，梵高家族即将迎来小生命。

过了几天，提奥又来信说，梵高的《葡萄园》卖出了400法郎的价格，随信把支票寄给他，这是梵高第一次拥有这么多钱，他再一次泪流满面。

他想为提奥做些什么，在医院的画室里，梵高为未出生的侄子画了这幅《杏花》。
#pagebreak()
#v(20em)
#align(center)[
  #set text(size: 24pt,font:"STXinwei")
  面对我们的骨灰，高尚的人们将洒下热泪。
]
#align(right)[  #set text(size: 22pt,font:"STXinwei")
  —— 卡尔·马克思  《青年选择职业的考虑》]
#pagebreak()
...
== Taylor定理
#idea[积分余项的Taylor公式]
#example[$ f in C^2[-1, 1], f(0)=0$, 求证:
$integral_(-1)^1 (f'')^2 dif x >= 10 (integral_(-1)^1 f(x) dif x)^2$]
#proof[

$ f(x) = f(0) + x f'(0) + integral_0^x f''(t)(x-t) dif t $
$
&integral_(-1)^1 f(x) dif x = integral_(-1)^1  integral_0^x f''(t)(x-t) dif t dif x\
&= integral_0^1 f''(t) integral_t^1 (x-t) dif x dif t - integral_(-1)^0 f''(t) integral_(-1)^t (x-t) dif x dif t\
&= integral_0^1 f''(t) (1-t)^2/2 dif t + integral_(-1)^0 f''(t) (1+t)^2/2 dif t\
&= integral_(-1)^1 f''(t) (1-|t|)^2/2 dif t $
$
(integral_(-1)^1 f(x) dif x)^2
<= integral_(-1)^1 (f'')^2 dif x integral_(-1)^1 ((1-|t|)^2/2)^2 dif t\
$

$ integral_(-1)^1 ((1-|t|)^2/2)^2 dif t = integral_(-1)^1 (1-|t|)^4/4 dif t = 1/2 integral_0^1 (1-t)^4 dif t = 1/10 $
...
=== 变量替换
#example[
$ text("计算") (x^2/a^2 + y^2/b^2)^2=x^2 + y^2 $
所围平面区域的面积.
]
令$x=a r cos theta,y=b r sin theta$,

则有$r^4=r^2(a^2 cos^2 theta + b^2 sin^2 theta)=>r^2=a^2 cos^2 theta + b^2 sin^2 theta$
$
  S=integral.double_D dif x dif y = integral_(0)^(2 pi) integral_0^sqrt(a^2 cos^2 theta + b^2 sin^2 theta) a b r dif r dif theta \
  = (a b)/2 integral_0^(2 pi) (a^2 cos^2 theta + b^2 sin^2 theta) dif theta = pi a b (a^2 + b^2)/2
$
#tip-box[$ integral_0^(2 pi)cos^2 theta dif theta = 1/2 integral_0^(2 pi) sin^2 theta + cos^2 theta dif theta = pi $]
=== 几种积分间的转化
#example[
  设区域$Omega$是由分段光滑曲面$Sigma$所围成,$u(x,y,z) in C^2(overline(Omega))$,且在$overline(Omega)$上调和，即$Delta u=0$，求证：
$  integral.double_Sigma (partial u)/(partial bold(n)) dif S =0
$
]
#proof[
  $
    (partial u)/(partial bold(n))=nabla u dot bold(n)\
    integral.double_Sigma (partial u)/(partial bold(n)) dif S = integral.double_Sigma nabla u dot bold(n) dif S\
    = integral.triple_Omega nabla dot (nabla u) dif V = integral.triple_Omega Delta u dif V =0
  $
]
...
#align(center)[
  #v(20em)
  #set text(size: 19pt,font:"STXinwei")
  卡拉玛佐夫即将死去，卡拉玛佐夫就要自杀。\ 人们会记住这一天的，\ 必须对得起他这颗诗人的灵魂,\ 也不枉他把自己的生命蜡烛是从两头儿一起点燃烧光的。
]
#align(right)[  #set text(size: 18pt,font:"STXinwei")
  ——陀思妥耶夫斯基   《卡拉马佐夫兄弟》]
#pagebreak()
= 傅里叶分析
== 基础知识
...
#align(center)[
  #v(20em)
  #set text(size: 25pt, font: "STXinwei")
  要相信啊：你的诞生绝非枉然！\
  你的生存和磨难绝非徒然！\
]
#align(right)[  #set text(size: 20pt, font: "STXinwei")
  ——马勒   《第二交响曲：复活》]
#pagebreak()
= 线性代数
== 线性无关性
...
#example[
  设 $X_i ~ chi^2(1)$, 独立同分布，$Y_n = sum_(i=1)^n X_i$，尝试使用上题的方法.
]
由中心极限定理知：
$
  (Y_n - n)/sqrt(2 n) ->^d text("N")(0,1) , quad (n->infinity)
$
因此
$
  P(Z<=0)=lim_(n->infinity) P((Y_n - n)/sqrt(2 n) <=0)=lim_(n->infinity) P(Y_n <= n)
$
注意到$Y_n ~ chi^2(n)$，因此
$
  P(Y_n <= n)&=integral_0^n (x^(n\/2 -1)e^(-x\/2))/(2^(n\/2) Gamma(n\/2)) dif x \
  &= 1/(2^(n/2) Gamma(n\/2)) integral_0^n x^(n\/2 -1)e^(-x\/2) dif x\
  &= 1/Gamma(n\/2) integral_0^(n\/2) t^(n\/2 -1)e^(-t) dif t \
  &= gamma(n\/2, n\/2)/Gamma(n\/2)
$
其中$ gamma(a,x)=integral_0^x t^(a-1) e^(-t) dif t,quad (a>0 ,x>0) $
故:
$
  lim_(n->infinity) gamma(n\/2, n\/2)/Gamma(n\/2) = 1/2
$
令$n$为偶数$n=2 m$,则
$
  lim_(m->infinity) gamma(m, m)/Gamma(m) = 1/2\
  lim_(m->infinity) 1/(m-1)! integral_0^m t^(m-1) e^(-t) dif t = 1/2
$
#remark[
  由于指数分布和卡方分布都是伽马分布的特例，使用指数分布也能得到这一结果.
]
== 随机过程的性质
#theorem[
  独立增量过程一定是马尔可夫过程，马尔可夫过程不一定是独立增量过程.
]
#example[
  $ X_1 ~ cal(N(0,1)) , X_n =2X_(n-1) $
  显然$X_n$是马尔可夫过程，但不是独立增量过程.
]

#pagebreak()
#align(center)[
  #v(20em)
  #set text(size: 25pt, weight: "bold",font: "STXinwei")
我终将遗忘梦境中的那些路径，山峦与田野，\
遗忘那些永远不能实现的梦。
]
#align(right)[  #set text(size: 22pt,font: "STXinwei")
...
#align(center)[
  #v(20em)
  #set text(size:28pt,font: "STXinwei")
  朝搴阰之木兰兮，夕揽洲之宿莽。
...
#align(center)[
  #v(20em)
  #set text(size:28pt, weight: "bold", font: "STXinwei")
...
在点 $theta_k$附近，损失函数 $f$ 可以用二阶泰勒展开近似为
$ f(theta) approx f_k + g_k^top (theta - theta_k) + 1/2 (theta - theta_k)^top H_k (theta - theta_k) $
我们希望找到一个增量 $delta = theta-theta_k$,让上面的二次近似最小.
$ (dif f)/(dif delta)=g_k + H_k delta = 0 $
$ delta = - H_k^(-1)g_k $
因此牛顿法二阶优化器的更新规则为 $theta_(k+1)=theta_k - H_k^(-1)g_k$.

然而，$(dif f)/(dif delta)=0$ 的点不一定是二次近似的最小值，可能是最大值或者鞍点.
#theorem-box[
当$H$为正定矩阵时,单步标准牛顿法必定会精确到达二阶泰勒近似的 Loss 的全局最小值.
]
设损失函数为$L(bold(theta))$, $L(bold(theta)_0 + Delta bold(theta)) approx L(bold(theta)_0) + nabla L(bold(theta)_0)^ top Delta bold(theta) + 1/2 Delta bold(theta)^top nabla^2 L(bold(theta_0)) Delta bold(theta)$

由于$bold(H)=nabla^2 L(bold(theta_0))$是实对称矩阵,所有特征值都是正数.

$ G(bold(theta)) =^Delta  L(bold(theta)_0) + nabla L(bold(theta)_0)^ top Delta bold(theta) + 1/2 Delta bold(theta)^top nabla^2 L(bold(theta_0)) Delta bold(theta) $

$ bold(H) = bold(Q Lambda Q^top),quad bold(Lambda)= text("diag")(lambda_1,dots,lambda_n) $

任意方向 $Delta bold(theta) = sum_i c_i bold(v_i)$，其中 $bold(v_i)$ 是 $bold(H)$ 的单位特征向量，$c_i$ 是系数.

$ Delta bold(theta)^top H Delta bold(theta) &= sum_i sum_j c_i c_j bold(v)_i^top H bold(v)_j\
&=sum_i lambda_i c_i^2  $

当 $bold(theta)$ 位于 $G$ 的临界点时，任意方向的增量都会导致 $Delta G =sum_i lambda_i c_i^2 > 0 $.
